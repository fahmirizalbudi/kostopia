...
func CancelRentalIfNotProceed(dbParam *sql.DB) error {
	sqlStatement := "UPDATE rentals SET status = 'cancelled' WHERE status = 'pending' AND created_at <= CURRENT_TIMESTAMP - INTERVAL '15 minutes' AND NOT EXISTS (SELECT 1 FROM transactions t WHERE t.rental_id = rentals.id);"
	_, err := dbParam.Exec(sqlStatement)
	return err
}
